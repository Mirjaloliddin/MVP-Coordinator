...
class BaseViewController<PresenterType>: UIViewController, Scene {
    private var _presenter: Presenter?
    var presenter: PresenterType {
        guard let presenter = _presenter as? PresenterType else {
            fatalError(
                """
                Failed to get presenter: noticed an attempt of using
                \(String(describing: self)) with nil or incorrect type of Presenter.
                """
            )
        }
        return presenter
